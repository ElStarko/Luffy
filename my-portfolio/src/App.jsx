...
const SKILLS = [
  {
    ...
    description:
      "Building scalable server-side applications with Express and RESTful APIs.",
    color: "#5C8A5A",
  },
  {
    icon: <Layers size={26} />,
  ...
  },
  {
    ...
    description:
      "Pandas, scikit-learn, and model evaluation for real datasets.",
    color: "#2E7D6E",
  },
  {
    icon: <Layers size={26} />,
    ...
    description:
      "React and Vite on the frontend, FastAPI and Express on the backend.",
    color: "#8C6A34",
  },
];
  ...
  return (
    <div className="dp-root">
      <style>{`
        ...
        .dp-root h1, .dp-root h2, .dp-root h3, .dp-root h4 { font-family: 'Fraunces', Georgia, serif; font-weight: 500; }
        ...
        .dp-h2 { font-size: clamp(1.8rem, 3.5vw, 2.4rem); margin: 0 0 1rem; position: relative; display: inline-block; }
        .dp-h2::after { content: ''; position: absolute; bottom: -10px; left: 0; width: 60px; height: 3px; border-radius: 2px; background: linear-gradient(90deg, var(--secondary), var(--accent)); }
        ...
        .dp-gradient-text { background: linear-gradient(135deg, var(--dark) 0%, var(--primary) 60%, var(--secondary) 100%); -webkit-background-clip: text; background-clip: text; color: transparent; }
        .dp-btn { display: inline-flex; align-items: center; justify-content: center; gap: 0.5rem; padding: 0.875rem 1.75rem; border: none; border-radius: 8px; font-weight: 600; font-size: 1rem; cursor: pointer; transition: var(--transition); text-decoration: none; font-family: 'IBM Plex Sans', sans-serif; }
        .dp-btn-primary { background: linear-gradient(135deg, var(--primary), var(--secondary)); color: white; box-shadow: var(--shadow); }
        ...
        .dp-header { position: sticky; top: 0; width: 100%; background: rgba(239,234,224,0.92); backdrop-filter: blur(10px); z-index: 50; padding: 1rem 0; box-shadow: var(--shadow); }
        ...
        .dp-nav-link { background: none; border: none; color: var(--dark); font-weight: 500; font-size: 0.95rem; cursor: pointer; padding: 0.4rem 0; position: relative; transition: var(--transition); font-family: 'IBM Plex Sans', sans-serif; }
        .dp-nav-link::after { content: ''; position: absolute; bottom: 0; left: 0; width: 0; height: 2px; background: var(--secondary); transition: var(--transition); }
        ...
        .dp-hero { padding: 6rem 0 5rem; position: relative; overflow: hidden; background: linear-gradient(135deg, #F3EFE5 0%, var(--light) 100%); }
        .dp-hero::before { content: ''; position: absolute; top: -50%; right: -20%; width: 600px; height: 600px; border-radius: 50%; background: radial-gradient(circle, rgba(140,106,52,0.14) 0%, transparent 70%); }
        .dp-hero-grid { display: grid; grid-template-columns: 1.1fr 0.9fr; gap: 3rem; align-items: center; position: relative; }
        ...
        .dp-panel { border-radius: 18px; overflow: hidden; box-shadow: var(--shadow-lg); transform: perspective(1000px) rotateY(-8deg); transition: var(--transition); border: 1px solid var(--gray-light); }
        ...
        .dp-expertise-item { display: flex; align-items: center; gap: 0.75rem; padding: 0.85rem; background: var(--card); border: 1px solid var(--gray-light); border-radius: 8px; transition: var(--transition); color: var(--dark); }
        .dp-expertise-item:hover { background: var(--primary); border-color: var(--primary); color: white; }
        .dp-about-panel { border-radius: 18px; padding: 2.5rem; background: linear-gradient(135deg, var(--primary), var(--dark)); color: white; box-shadow: var(--shadow-lg); }
        .dp-skills { background: linear-gradient(135deg, #F3EFE5 0%, var(--light) 100%); }
        .dp-grid-3 { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 2rem; }
        .dp-skill-card { background: var(--card); padding: 2rem; border-radius: 14px; box-shadow: var(--shadow); transition: var(--transition); border-top: 4px solid var(--skill-color); border-left: 1px solid var(--gray-light); border-right: 1px solid var(--gray-light); border-bottom: 1px solid var(--gray-light); }
        ...
        .dp-project-overlay { position: absolute; inset: 0; background: rgba(23,21,18,0.78); display: flex; align-items: center; justify-content: center; opacity: 0; transition: var(--transition); }
        .dp-project-card:hover .dp-project-overlay { opacity: 1; }
        .dp-project-link { display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.6rem 1.1rem; background: var(--secondary); color: white; text-decoration: none; border-radius: 8px; font-size: 0.9rem; font-weight: 600; }
        ...
        .dp-tag { background: var(--light); color: var(--primary); border: 1px solid var(--gray-light); padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.78rem; font-weight: 500; }
        .dp-contact { background: linear-gradient(135deg, var(--dark) 0%, var(--dark-light) 100%); color: white; }
        .dp-contact .dp-h2 { color: white; }
        .dp-contact .dp-h2::after { background: linear-gradient(90deg, var(--secondary), var(--accent)); left: 50%; transform: translateX(-50%); }
        .dp-contact-card { background: rgba(255,255,255,0.05); padding: 2.5rem; border-radius: 14px; backdrop-filter: blur(10px); max-width: 560px; margin: 0 auto; border: 1px solid rgba(255,255,255,0.08); }
        .dp-contact-item { display: flex; align-items: flex-start; gap: 1rem; margin-bottom: 1.5rem; }
        .dp-contact-icon { width: 42px; height: 42px; background: rgba(176,86,47,0.16); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: var(--accent); flex-shrink: 0; }
        ...
        .dp-social a { display: inline-flex; align-items: center; justify-content: center; width: 44px; height: 44px; background: rgba(255,255,255,0.08); border-radius: 50%; color: white; transition: var(--transition); }
        .dp-social a:hover { background: var(--secondary); transform: translateY(-3px); }
        .dp-footer { background: var(--dark); color: white; padding: 3rem 0 2rem; text-align: center; }
        .dp-footer-bottom { border-top: 1px solid rgba(255,255,255,0.1); padding-top: 1.5rem; margin-top: 1.5rem; color: var(--gray-light); font-size: 0.88rem; font-family: 'IBM Plex Sans', sans-serif; }
      ...
      <header className="dp-header">
        <div className="dp-container dp-nav">
          ...
          <ul className="dp-nav-links">
            <li>
              <button className="dp-nav-link" onClick={() => scrollTo("#hero")}>
                Home
              </button>
            </li>
            <li>
        ...
        {menuOpen && (
          ...
          >
            ...
            <button className="dp-nav-link" onClick={() => scrollTo("#about")}>
              About
            </button>
            <button className="dp-nav-link" onClick={() => scrollTo("#skills")}>
              Skills
            </button>
            <button
              className="dp-nav-link"
        ...
        <div className="dp-container dp-hero-grid">
          <div>
            ...
            <p className="dp-hero-desc">
              I build scalable server-side solutions and extract insights from
              complex datasets — specializing in Node.js, Python, API
              development, and applied machine learning, most recently on a
              system for detecting electoral malpractice.
            </p>
          ...
          <div className="dp-panel">
            <div className="dp-code-card">
              ...
              <div>
                &nbsp;&nbsp;name:{" "}
                <span style={{ color: "#2E7D6E" }}>'Johnson Iloabuchi'</span>,
              </div>
              <div>
                &nbsp;&nbsp;role:{" "}
                <span style={{ color: "#2E7D6E" }}>'Backend + ML'</span>,
              </div>
              <div>
                &nbsp;&nbsp;stack: [
                <span style={{ color: "#C98A4B" }}>'Node'</span>,{" "}
                <span style={{ color: "#C98A4B" }}>'Python'</span>,{" "}
                <span style={{ color: "#C98A4B" }}>'React'</span>],
              </div>
              <div>
                &nbsp;&nbsp;available:{" "}
                <span style={{ color: "#5C8A5A" }}>true</span>
              </div>
              <div>{"}"};</div>
            </div>
          </div>
      ...
      <section id="about" className="dp-section">
        <div className="dp-container dp-about-grid">
          <div>
            ...
            >
              My background is on the backend and data side — Node.js and
              Express APIs, database design, and real-time systems with
              Socket.io. That's the foundation behind a booking platform, a
              payment gateway, and a live-chat app I've built. More recently
              I've moved into applied ML for my thesis: a full
              electoral-malpractice detection system covering classification,
          ...
          <div className="dp-about-panel">
            ...
            >
              Looking for roles where I can keep working across the data,
              backend, and frontend layers of a product.
            </p>
          </div>
        </div>
      </section>
